...
const SEVERITY_COLOR: Record<AnnouncementType, { bg: string; bgDark: string; accent: string; text: string }> = {
  info:    { bg: 'rgba(59,130,246,0.07)',  bgDark: 'rgba(59,130,246,0.12)',  accent: '#3B82F6', text: '#1D4ED8' },
  warning: { bg: 'rgba(245,158,11,0.08)',  bgDark: 'rgba(245,158,11,0.13)',  accent: '#F59E0B', text: '#92400E' },
  error:   { bg: 'rgba(239,68,68,0.07)',   bgDark: 'rgba(239,68,68,0.12)',   accent: '#EF4444', text: '#991B1B' },
  success: { bg: 'rgba(34,197,94,0.07)',   bgDark: 'rgba(34,197,94,0.11)',   accent: '#22C55E', text: '#14532D' },
};
...
const AnnouncementBanner: React.FC = () => {
  ...
  const handleLink = (url: string) => {
    url.startsWith('http') ? window.open(url, '_blank', 'noopener') : navigate(url);
  };
      ...
      >
        ...
        {total > 1 && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.2, flexShrink: 0 }}>
            <IconButton size="small" sx={{ p: 0.3 }} onClick={() => setIndex(i => (i - 1 + total) % total)} disabled={total <= 1}>
              <PrevIcon sx={{ fontSize: 16 }} />
            </IconButton>
            <Typography variant="caption" sx={{ fontSize: '0.7rem', color: 'text.secondary', minWidth: 28, textAlign: 'center' }}>
              {Math.min(index, total - 1) + 1}/{total}
            </Typography>
            <IconButton size="small" sx={{ p: 0.3 }} onClick={() => setIndex(i => (i + 1) % total)} disabled={total <= 1}>
        ...
        {/* 关闭按钮 */}
        {current.dismissible && (
          <IconButton size="small" onClick={handleDismiss} sx={{ p: 0.4, flexShrink: 0, opacity: 0.6, '&:hover': { opacity: 1 } }}>
