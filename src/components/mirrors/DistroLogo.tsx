  ...
  openeuler: (size, isDark) => (
    <svg viewBox="0 0 1024 1024" width={size} height={size} aria-label="openEuler">
      <path
        ...
        // openEuler 品牌色 #002FA7 亮度极低，深色模式下替换为浅蓝色
        fill={isDark ? '#6B9FFF' : '#002FA7'}
      ></path>
    </svg>
  ),
};
...
const DistroLogo: React.FC<DistroLogoProps> = ({ id, size = 22 }) => {
  ...
  // 1. simple-icons 本地导入
  if (info) {
    // 在深色模式下，亮度过低的品牌色（如 CentOS #262577、Alpine #0D597F）会不可见
    // 对这些颜色在深色模式下替换为白色以保证可读性
    const luminance = getRelativeLuminance(info.color);
    const fillColor = isDark && luminance < DARK_LUMINANCE_THRESHOLD ? '#FFFFFF' : `#${info.color}`;
